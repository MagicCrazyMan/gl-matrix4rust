...
impl AsRef<[u8]> for Mat2d<f32> {
    fn as_ref(&self) -> &[u8] {
        unsafe { std::mem::transmute::<&[f32; 6], &[u8; 24]>(&self.0) }
    }
}

impl AsRef<[u8]> for Mat2d<f16> {
    fn as_ref(&self) -> &[u8] {
        unsafe { std::mem::transmute::<&[f16; 6], &[u8; 12]>(&self.0) }
    }
}
...
mod tests {
    ...
    static MAT_A: OnceLock<Mat2d<f32>> = OnceLock::new();
    static MAT_B: OnceLock<Mat2d<f32>> = OnceLock::new();

    fn mat_a() -> &'static Mat2d<f32> {
        MAT_A.get_or_init(|| Mat2d::from_slice(&MAT_A_RAW))
    }

    fn mat_b() -> &'static Mat2d<f32> {
